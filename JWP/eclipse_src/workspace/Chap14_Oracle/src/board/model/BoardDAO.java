...
public class BoardDAO {
	...
	public void boardWrite(String name, String subject, String content, String password){
		...
		try{
			...
			sql = "INSERT INTO BOARD (num, name, password, subject, content, write_date, write_time, ref, step, lev, read_cnt, child_cnt) values(?, ?, ?, ?, ?, CURRENT_DATE, CURRENT_TIMESTAMP, ?, 0, 0, 0, 0)";
			pstmt = conn.prepareStatement(sql);
	...
	public int boardReplySearchStep(String ref, String lev, String step){
		...
		try{
			conn = ds.getConnection();
			String sql = "SELECT nvl(MIN(step), 0) from BOARD WHERE ref = ? and lev <= ? and step > ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, Integer.parseInt(ref));
